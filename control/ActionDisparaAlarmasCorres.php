...
include_once(dirname(__FILE__).'/../../sis_correspondencia/modelo/MODCorrespondencia.php');
		...
		foreach ($res2->datos as $d){
				...
				try {	
						...
						if(isset($d['email_empresa'])){
							...
							//$correo->addDestinatario('rosanavq@gmail.com','rosanavq@gmail.com'); 
...
/*		$objParam->addParametro('errores_id', $errores_id);
        ...
        $objFunc=new MODCorrespondencia($objParam);        
        $res2=$objFunc->eliminarAlarmaAnulada();
		if($res2->getTipo()=='ERROR'){
			echo 'Se ha producido un error-> Mensaje Técnico:'.$res2->getMensajeTec();
			exit;        
		}

...
BEGIN
    	...
    	begin   
              ...
              IF p_administrador = 1 THEN
                ...
                ELSE
                     ...
                     select permitir_todo
                            into v_permitir_todo
               ...
               --  raise exception '%',v_filtro;
                ...
                ELSE
                     ...
                     if (v_permitir_todo ='si') then
                         ...
                         --v_filtro=v_filtro;
                         v_filtro = v_filtro||' and (cor.id_funcionario = ' ||v_parametros.id_funcionario_usuario || ' or cor.id_usuario_reg = '||p_id_usuario||')';
                         
                    end if;           
                 -- v_filtro = '0=0';
                  --v_filtro = ' (cor.id_funcionario = ' ||v_parametros.id_funcionario_usuario::varchar ||' or cor.id_usuario_reg = '||p_id_usuario||')'; 
         ...
         END IF;
  		...
  		--Sentencia de la consulta
			v_consulta:='select
                        ...
                        emp_recepciona.desc_funcionario1 as des_funcionario_origen,
                        (CASE WHEN (cor.id_acciones is not null) then
   ...
   -- raise exception '%','dfgasdf';
         ...
         IF  v_parametros.tipo = 'externa' THEN
             ...
             END IF;
          IF (v_parametros.interface = 'recibida_archivada')THEN
              v_filtro= v_filtro||' and   cor.estado in (''pendiente_recibido'',''recibido'',''recibido_derivacion'',''enviado'')  ';
          ELSE
              v_filtro= v_filtro||' and   cor.estado in (''pendiente_recibido'',''recibido'',''recibido_derivacion'')  ';
...
--raise exception '%',v_parametros.estado;
            ...
            ELSIF v_parametros.estado = 'anulado' THEN
				v_filtro= v_filtro || ' and cor.estado in (''anulado'') and ';
...
--            IF (v_parametros.tipo is NULL) THEN
	           ...
	           -- v_filtro=v_filtro || ' cor.tipo='''||v_parametros.tipo||''' and';
/*                
            ELSE*/
            
	           -- v_filtro=v_filtro || ' cor.tipo='''||v_parametros.interface||''' and';
    /*        END IF;
*/
			--Sentencia de la consulta
			v_consulta:='select
                        ...
                        inner join segu.tclasificador clasif on clasif.id_clasificador=cor.id_clasificador
						left join segu.tusuario usu2 on usu2.id_usuario = cor.id_usuario_mod
				        where cor.id_correspondencia_fk is null and '||v_filtro||'    
                         ';
    ...
    ***********************************/
		...
		begin
			...
			v_consulta:='select count(id_correspondencia)
                        ...
                        inner join segu.tclasificador clasif on clasif.id_clasificador=cor.id_clasificador
						left join segu.tusuario usu2 on usu2.id_usuario = cor.id_usuario_mod
				        where cor.tipo=''externa'' and cor.id_correspondencia_fk is null and  '||v_filtro||'  ';

    	...
    	begin
          ...
          --Sentencia de la consulta
			v_consulta:='select
						...
						cor.tipo,
						cor.fecha_reg,						
						coalesce(funcionario.desc_funcionario1,coalesce(ins.nombre,per.nombre)) as desc_funcionario
                        from corres.tcorrespondencia cor
						left join orga.vfuncionario funcionario on funcionario.id_funcionario=cor.id_funcionario
               		    left join param.tinstitucion ins on ins.id_institucion=cor.id_institucion
		...
		begin
    		...
    		IF pxp.f_existe_parametro(p_tabla,'interface') THEN
                       ...
                       /* IF v_parametros.interface = 'externa' THEN
                        ...
                        IF v_parametros.interface = 'saliente' THEN
                            v_filtro= v_filtro || ' and cor.tipo = ''saliente'' ';
                        END IF;*/
